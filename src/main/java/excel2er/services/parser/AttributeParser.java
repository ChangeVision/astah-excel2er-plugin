...
public class AttributeParser {
	...
	public void parse(Configuration configuration, Sheet sheet,
			Entity entity) {
		int startRow = NumberUtils.toInt(configuration.getStartRow());
		
		while(true){
			
			String logicalName = ParserUtils.getCellValue(sheet,startRow,configuration.getAttributeLogicalCol());
			String physicalName = ParserUtils.getCellValue(sheet,startRow,configuration.getAttributePhysicalCol());
			...
			parsePrimaryKey(configuration, sheet,
					startRow, attr);
	...
	private void parseDefinition(Configuration configuration, Sheet sheet,
		...
		String value = ParserUtils.getCellValue(sheet,startRow, configuration.getDefinitionCol());
	...
	private void parseDefaultValue(Configuration configuration, Sheet sheet,
			int startRow, Attribute attr) {
		String value = ParserUtils.getCellValue(sheet,startRow, configuration.getDefaultValueCol());
		if(StringUtils.isNotEmpty(value)){
	...
	private void parseLength(Configuration configuration, Sheet sheet,
			int startRow, Attribute attr) {
		String value = ParserUtils.getCellValue(sheet,startRow, configuration.getLengthCol());
		if(StringUtils.isNotEmpty(value)){
			attr.setLength(value);
		}
	}

	void parseDataType(Configuration configuration,
			Sheet sheet, int startRow, Attribute attr) {
	...
	void parseNotNull(Configuration configuration,
			Sheet sheet, int startRow, Attribute attr) {
	...
	void parsePrimaryKey(Configuration configuration,
			Sheet sheet, int startRow, Attribute attr) {
		String value = ParserUtils.getCellValue(sheet, startRow, configuration.getPrimaryKeyCol());
		if(StringUtils.isNotEmpty(value)){
