...
public class EntityPanelTest {
	...
	public void should_be_able_to_select_single_radiobutton() throws Exception {
		...
		panelFixture.radioButton(EntityPanel.AdvanceSettingButton.NAME).requireNotSelected();

		panelFixture.radioButton(EntityPanel.AdvanceSettingButton.NAME).click();
		panelFixture.radioButton(EntityPanel.UseSheetNameButton.NAME).requireNotSelected();
	}
	
	@Test
	public void should_get_usesheetname_button_is_selected() throws Exception {
		panelFixture.radioButton(EntityPanel.UseSheetNameButton.NAME).click();
		assertThat(((EntityPanel)panelFixture.target).isUseSheetName(),is(true));
		
		panelFixture.radioButton(EntityPanel.AdvanceSettingButton.NAME).click();
		assertThat(((EntityPanel)panelFixture.target).isUseSheetName(),is(false));
	}
	
	@Test
	public void should_get_advancesetting_button_is_selected() throws Exception {
		panelFixture.radioButton(EntityPanel.AdvanceSettingButton.NAME).click();
		assertThat(((EntityPanel)panelFixture.target).isAdvanceSetting(),is(true));
		
		panelFixture.radioButton(EntityPanel.UseSheetNameButton.NAME).click();
		assertThat(((EntityPanel)panelFixture.target).isAdvanceSetting(),is(false));
	}
	...
	public void should_get_logical_row_and_col() throws Exception {
		...
		panelFixture.textBox(EntityPanel.AdvanceElementRowCol.LOGICAL_ROW).setText("3");
		panelFixture.textBox(EntityPanel.AdvanceElementRowCol.LOGICAL_COL).setText("4");
	...
	public void should_get_physical_row_and_col() throws Exception {
		...
		panelFixture.textBox(EntityPanel.AdvanceElementRowCol.PHYSICAL_ROW).setText("8");
		panelFixture.textBox(EntityPanel.AdvanceElementRowCol.PHYSICAL_COL).setText("9");
		
		assertThat(((EntityPanel)panelFixture.target).getPhysicalRow(),is("8"));
		assertThat(((EntityPanel)panelFixture.target).getPhysicalCol(),is("9"));
	}
}
